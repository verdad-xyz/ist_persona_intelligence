...
export const createFraudName = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Fraud name berhasil dibuat", data: fraudName });
  } catch (error) {
    console.error(error.message);
...
export const updateFraudName = async (req, res) => {
  try {
    ...
    const existing = await FraudName.findOne({ where: { name } });
...
export const updateFraudCategory = async (req, res) => {
  try {
    ...
    const existing = await FraudCategory.findOne({ where: { name } });
